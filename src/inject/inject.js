...
(function(window, document, $) {
    ...
    ColumnCopy.prototype.bindHandlers = function() {
        ...
        $(document).on('keyup', null, '', function() {
            that.activeHotkeysReset();
        });

        // Set the column hotkey when its combo is pressed. Same for table hotkey.
        $(document).on('keydown', null, this.options.columnHotkey, function() {
            that.activeHotkey.column = true;
        });
        $(document).on('keydown', null, this.options.tableHotkey, function() {
            that.activeHotkey.table = true;
        });

        // Finally, execute the routine on click
        $(document).on('click', 'th,td', function(e) {
            that.handleCellClick(e, this);
        });
            ...
            $('>td, >th', this).each(function() {
                ...
                for (i = map.length - 1; i >= 0; i--) {
                    if (cellMap.indexOf(map[i]) !== -1) {
                        var cellText = that.getCellText($this[0]);
    ...
    ColumnCopy.prototype.copyValuesToClipboard = function(values) {
        ...
        chrome.extension.sendMessage({
            toCopy: values.join(this.options.rowSeparator)
        });
    };
